...
export class createProject extends Component {
    ...
    NotifyTaskCreationSuccess = () => {
        if (!toast.isActive(this.toastId)) {
            this.toastId = toast.success(<center>Project Created Successfully</center>, {
    ...
    method(data) {
        ...
        if (data.length > 2) {
            console.log("method", data)
            Axios.get('http://localhost:8080/get-emails-while-search?email=' + data).then((response) => {
                if (response.data.statusCode == 201) {
                    ...
                    console.log(this.state.emailLists.length)
