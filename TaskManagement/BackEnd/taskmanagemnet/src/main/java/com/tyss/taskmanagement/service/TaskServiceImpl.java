...
public class TaskServiceImpl implements TaskService {
	...
	public Response updateStatus(int taskId, String status) {
		...
		try {
			...
			if (taskbean != null) {
				...
				response=ResponseContainerutil.fillerSuccess("Status Changed successfully");
			} else {
				response=ResponseContainerutil.fillerFailure("Status not Changed");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
		return response;
	}
	 ...
	 */
	@Override
	public Response getAssignToTask(String email,HttpServletRequest request) {
		Response response =null;
		if(request.getSession(false) != null) {
		 List<TaskBean> taskBeans= taskRepository.getAssignTo(email);
			if (!taskBeans.isEmpty()) {
				response=ResponseContainerutil.fillerSuccess("All AssignTo Task data assigned Found ");
				response.setTaskBean(taskRepository.getAssignTo(email));
				
			} else {
				response=ResponseContainerutil.fillerFailure("AssignTo Task data not found");
			}
		}else {
			response=ResponseContainerutil.fillerFailure("HttpServletRequest  is not valid");
		}
		return response;
	...
	public Response getAssignedTask(String email) {
		...
		try {
			...
			if (!beans.isEmpty()) {
				...
				response=ResponseContainerutil.fillerFailure("Assigned Task data not found");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
	...
	public Response getCompletedTaskByMe(String email, String from) {
		...
		try {
			...
			if (!endList.isEmpty()) {
				response=ResponseContainerutil.fillerSuccess("Completed task by me found successfully");
				...
				response=ResponseContainerutil.fillerFailure("Completed task by me not found");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
	...
	public Response getCompletedTaskToMe(String email, String from) {
		...
		try {
			if (userRepository.existsByEmail(email)) {
				response=ResponseContainerutil.fillerSuccess("All Completed Task to me found Successfully");
				TreeSet<TaskBean> set = new TreeSet<>();
				...
				response=ResponseContainerutil.fillerFailure("All Completed Task to me not found");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
	...
	public Response searchTaskToMe(String data, String email) {
		...
		try {
			if (taskRepository.countSubject(data) > 0 || taskRepository.countDescription(data) > 0) {
				response=ResponseContainerutil.fillerSuccess("search Task to me found successfully");
				response.setTaskBean(taskRepository.findToMe(data, email));
			} else {
				response=ResponseContainerutil.fillerFailure("search Task data not found");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
	...
	public Response searchTaskByMe(String data, String email) {
		...
		try {
			if (taskRepository.countSubject(data) > 0 || taskRepository.countDescription(data) > 0) {
				response=ResponseContainerutil.fillerSuccess("search Task by me found successfully");
				response.setTaskBean(taskRepository.findByMe(data));
			} else {
				response=ResponseContainerutil.fillerFailure("search Task by me not found");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
	...
	public Response getCompletedTaskByDate(String from, String to, String email) {
		...
		try {
			if (userRepository.existsByEmail(email)) {
				response=ResponseContainerutil.fillerSuccess("Task by date  found successfully");
				response.setEnd(taskRepository.fromTo(email, from, to));
			} else {
				response=ResponseContainerutil.fillerFailure("Task by date data not found");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
	...
	public Response updateCompletedDate(int taskId, String status, String completedDate) {
		...
		try {
			...
			if (taskbean != null) {
				...
				taskRepository.save(taskbean);
				response=ResponseContainerutil.fillerSuccess("CompletedDate Change successfully");
			} else {
				response=ResponseContainerutil.fillerFailure("Status not Changed");
			}
		} catch (Exception e) {
	...
	public Response getTaskForProject(int projectId,String email) {
		...
		if (!taskRepository.findProject(projectId).isEmpty()) {
			response=ResponseContainerutil.fillerSuccess("All Task Assigned for project Found Successfully");
			response.setBean(userRepository.getUserBean(email).get());
			response.setTaskBean(taskRepository.findProject(projectId));
		} else {
			response=ResponseContainerutil.fillerFailure("project not Found");
		}
		return response;
	}
	...
	public Response getCompletedTaskForProject(int pid, String from) {
			...
			if (!beans.isEmpty()) {
				response=ResponseContainerutil.fillerSuccess("Completed Task Found");
				response.setTaskBean(beans);
				response.setEnd(taskRepository.findCompletedDateForProject(pid, from));
			} else {
				response=ResponseContainerutil.fillerFailure("Completed Task  not Found");
			}
			return response;		
	}
	...
	public Response getCompletedProjectTaskByDate(String from, String to, int projectId) {
		...
		try {
			...
			if (!beans.isEmpty()) {
				response=ResponseContainerutil.fillerSuccess("Completed Task for project Found ");
				response.setEnd(beans);
			} else {
				response=ResponseContainerutil.fillerFailure("Completed Task for project not Found ");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
	...
	public Response  updateTaskInfo( int taskId, String description, String assignedTo, String deadLine, String priority, String subject) {
		...
		try {
			...
			if (taskbean != null) {
				...
				taskRepository.save(taskbean);
				response=ResponseContainerutil.fillerSuccess("Task updated successfully");
			} else {
				response=ResponseContainerutil.fillerFailure("Task updated  not Changed");
			}
		} catch (Exception e) {
			response=ResponseContainerutil.fillerException( e.getMessage());
		}
